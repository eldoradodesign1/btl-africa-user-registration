-- BTL Africa: keep the existing direct password storage contract.
-- The only security change requested here is requiring the current password
-- before a profile update. Existing bcrypt values remain login-compatible.
  ...
  return query
    ...
    where u.phone = p_phone
      and (
        ...
        or (u.password_hash ~ '^\$2[aby]\$' and crypt(p_password, u.password_hash) = u.password_hash)
      );
end;
$function$;
...
-- Keep admin-created users in the same direct password format as before.
...
begin
  ...
  values (p_id, trim(p_full_name), p_phone, p_password, p_role, p_user_category, p_supervisor_id, p_permanent_shop_id);
...
-- Keep direct storage, but require the current password before profile changes.
...
begin
  ...
  if current_hash is null or p_current_password is null or not (
    current_hash = p_current_password
    or (current_hash ~ '^\$2[aby]\$' and crypt(p_current_password, current_hash) = current_hash)
  ) then
...
-- New signup requests are stored exactly as before.
