-- BTL Africa: allow administrative staff to create agent accounts directly.
-- Registration-request approval remains restricted to super_admin in the existing flow.
-- Admin and super_admin keep their existing ability to create any allowed role.
-- supervisor and sub_admin may create agents only.
...
begin
  ...
  if creator_role not in ('admin', 'super_admin', 'supervisor', 'sub_admin') then
  ...
  if p_phone is null or p_phone !~ '^(08|09)[0-9]{8}$' then
