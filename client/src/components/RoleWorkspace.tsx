...
import { BriefcaseBusiness, CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, Download, FilePenLine, FileSpreadsheet, FileText, LoaderCircle, LockKeyhole, UserCircle2, Users, X, XCircle } from "lucide-react";
...
function AttendanceCalendar({ insights }: { insights: AgentInsights }) {
  ...
  const presence = new Map(insights.presence.map((entry) => [entry.date, entry]));
...
export function ProfileModal({ profile, onClose, onSaved }: { profile: UserRecord; onClose: () => void; onSaved: (profile: UserRecord) => void }) {
  ...
  async function save(event: FormEvent) {
    ...
    setSaving(true);
    try { onSaved(await updateMyProfile({ fullName, phone, currentPassword, password: password || undefined, avatarUrl })); onClose(); } catch (err) { setError(err instanceof Error ? err.message : "Impossible de mettre à jour le profil."); } finally { setSaving(false); }
  }
