...
export function UserDetailModal({ user, users = [], superiors, campaigns, assignments, assignmentRequests, requester, canRequest, onNotice, onClose }: UserDetailProps) {
...
export function AgentDetailModal({ agent, users, campaigns, assignments, assignmentRequests, canRequest, canExport, canEdit, onEdit, onNotice, onClose }: AgentDetailProps) {
...
export default function RoleWorkspace({ profile, users, superiors, campaigns, assignments, assignmentRequests, campaignClaims, onNotice, onProfileUpdated, onProfileOpen, onRequestReviewed, simulation = false }: RoleWorkspaceProps) {
  ...
  const superiorDirectory = Array.from(new Map([...superiors, ...users.filter((user) => ["supervisor", "sub_admin", "admin", "super_admin"].includes(user.role))].map((user) => [user.id, user])).values()).sort((a, b) => a.full_name.localeCompare(b.full_name));
  ...
  return <section className="role-workspace">
    ...
    {selectedAgent && <AgentDetailModal agent={selectedAgent} users={users} campaigns={campaigns} assignments={assignments} assignmentRequests={assignmentRequests} canRequest={false} canExport onNotice={onNotice} onClose={() => setSelectedAgent(null)} />}{selectedSuperior && <UserDetailModal user={selectedSuperior} users={users} superiors={superiorDirectory} campaigns={campaigns} assignments={assignments} assignmentRequests={assignmentRequests} requester={profile} canRequest={isAgent && !simulation} onNotice={onNotice} onClose={() => setSelectedSuperior(null)} />}{photoPreview && <ProfilePhotoPreviewModal user={profile} onClose={() => setPhotoPreview(false)} />}
  </section>;
}
