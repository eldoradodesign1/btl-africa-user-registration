...
import { loadAgentInsights, requestCampaignAssignment, reviewCampaignAssignmentRequest, updateMyProfile, type AgentInsights, type CampaignAssignment, type CampaignAssignmentRequest, type CampaignRecord, type DailyReport, type UserRecord } from "@/lib/supabase";
...
export function ProfilePhotoPreviewModal({ user, onClose }: { user: UserRecord; onClose: () => void }) {
  return <div className="modal-layer profile-photo-lightbox"><button className="modal-backdrop" type="button" aria-label="Fermer la photo" onClick={onClose} /><div className="profile-photo-lightbox-card"><button type="button" className="modal-close" onClick={onClose} aria-label="Fermer"><X size={16} /></button><Avatar user={user} size="large" /><strong>{user.full_name}</strong><small>{ROLE_LABELS[user.role]}</small></div></div>;
}
