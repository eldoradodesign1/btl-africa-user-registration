...
import { BriefcaseBusiness, CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, Download, FileSpreadsheet, FileText, LoaderCircle, LockKeyhole, UserCircle2, Users, X, XCircle } from "lucide-react";
...
function AttendanceCalendar({ insights }: { insights: AgentInsights }) {
  const firstDate = insights.presence[0]?.date || insights.campaignStart || new Date().toISOString().slice(0, 10);
  ...
  const isPause = (date: string) => insights.pauses.some((pause) => date >= pause.starts_on && date <= pause.ends_on);
  const isInCampaign = (date: string) => (!insights.campaignStart || date >= insights.campaignStart) && (!insights.campaignEnd || date <= insights.campaignEnd) && !isPause(date);
