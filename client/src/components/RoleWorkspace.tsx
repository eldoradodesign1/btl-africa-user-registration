...
import { AlertTriangle, BriefcaseBusiness, CalendarDays, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight, Clock3, Download, FilePenLine, FileSpreadsheet, FileText, History, LoaderCircle, LockKeyhole, MessageCircle, PhoneCall, Send, UserCircle2, Users, X, XCircle } from "lucide-react";
...
export function CampaignClaimCaseModal({ claim, viewer, agent, campaign, users, canManage, onNotice, onChanged, onClose }: { claim: CampaignClaim; viewer: UserRecord; agent?: UserRecord; campaign?: CampaignRecord; users: UserRecord[]; canManage: boolean; onNotice: (notice: Notice) => void; onChanged?: (claim: CampaignClaim) => void; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    void loadCampaignClaimMessages(viewer.id, claim.id).then((next) => { if (!cancelled) setMessages(next); }).catch((error) => { if (!cancelled) onNotice({ kind: "error", message: readableSupabaseError(error, "Impossible de charger l’historique du dossier.") }); }).finally(() => { if (!cancelled) setLoadingMessages(false); });
    if (!canManage && claim.unread_count) void markCampaignClaimRead(viewer.id, claim.id).catch(() => undefined);
    return () => { cancelled = true; };
  }, [claim.id, claim.unread_count, canManage, onNotice, viewer.id]);
  ...
  async function transition(status: CampaignClaimStatus) {
    if (["awaiting_agent", "resolved", "rejected"].includes(status) && statusNote.trim().length < 3) {
