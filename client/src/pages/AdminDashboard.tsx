import { useEffect, useMemo, useState, type ChangeEvent, type CSSProperties, type FormEvent, type KeyboardEvent as ReactKeyboardEvent } from "react";
import { createPortal } from "react-dom";
import { AlertCircle, BriefcaseBusiness, CalendarDays, CheckCircle2, ChevronDown, ChevronLeft, Clock3, Database, Download, FilePenLine, FileSpreadsheet, FileText, Filter, ImagePlus, LayoutGrid, List, LoaderCircle, LogIn, LogOut, MapPin, PieChart, RefreshCw, Search, ServerCog, ShieldCheck, Trash2, UserCheck, UserPlus, UserCircle2, UserRoundPlus, X, XCircle } from "lucide-react";
import { CopyablePhone } from "@/components/CopyablePhone";
import RoleWorkspace, { AgentDetailModal, Avatar, ProfileModal, ProfilePhotoPreviewModal, UserDetailModal } from "@/components/RoleWorkspace";
...
function supervisorDisplayName(supervisorId: string | null, users: UserRecord[]): string {
...
function UserGalleryCard({ user, users, campaigns, assignments, campaignSupervisorAssignments, shops, canManage, canManageCampaigns, onOpenUser, onOpenPhoto, onAssign, onEdit, onDelete }: UserGalleryCardProps) {
  const [revealed, setRevealed] = useState(false);
  const userCampaignIds = new Set([
    ...assignments.filter((assignment) => assignment.user_id === user.id && assignment.is_active).map((assignment) => assignment.campaign_id),
  ...
  const userCampaigns = campaigns.filter((campaign) => userCampaignIds.has(campaign.id));
  ...
  return <article className={`user-gallery-card ${revealed ? "is-revealed" : ""}`} tabIndex={0} role="button" aria-expanded={revealed} aria-label={`${revealed ? "Ouvrir la fiche de" : "Révéler les informations de"} ${user.full_name}`} onClick={toggleCard} onKeyDown={handleKeyDown}>
    <div className="user-gallery-track">
      ...
      <div className="user-gallery-info" aria-hidden={!revealed}>
        <div className="user-gallery-info-head"><div><strong>{user.full_name}</strong><small>{category}</small></div><div className="user-gallery-info-head-actions"><button type="button" className="user-gallery-back" onClick={(event) => { event.stopPropagation(); setRevealed(false); }} aria-label="Revenir à la photo" title="Revenir à la photo"><ChevronLeft size={13} /><span>Photo</span></button><span className={`role-badge role-${user.role}`}>{ROLE_LABELS[user.role]}</span></div></div>
        <div className="user-gallery-contact"><CopyablePhone value={user.phone} className="mono-value" /><span>{user.id.slice(0, 8)}…</span></div>
        <div className="user-gallery-facts"><div><small>Campagnes</small><div className="campaign-pills">{userCampaigns.length ? userCampaigns.map((campaign) => <span className="campaign-pill" key={campaign.id} title={campaign.name}>{campaign.name}</span>) : <span className="muted-note">Aucune</span>}</div></div><div><small>Entrée agence</small><strong>{supervisorDisplayName(user.supervisor_id, users)}</strong></div><div><small>Shop</small><strong>{shopDisplayName(user.permanent_shop_id, shops)}</strong></div></div>
...
function AdminDashboard({ onConnectionChanged, onRequestCreate }: Props) {
  ...
  async function refreshUsers(currentProfile: UserRecord | null = profile) {
    setLoading(true);
    try {
      const [nextUsers, nextSuperiors, nextCampaigns, nextShops, nextAssignments, nextCampaignSupervisorAssignments, nextRequests, nextClaims] = await Promise.all([loadUsers(), loadSupervisors(), loadCampaigns(), loadShops().catch(() => []), loadCampaignAssignments(), loadCampaignSupervisorAssignments().catch(() => []), currentProfile && ["admin", "super_admin", "sub_admin", "supervisor"].includes(currentProfile.role) ? loadCampaignAssignmentRequests() : Promise.resolve([]), currentProfile && ["admin", "super_admin", "sub_admin", "supervisor"].includes(currentProfile.role) ? loadCampaignClaims().catch(() => []) : Promise.resolve([])]);
      setUsers(nextUsers);
  ...
  async function handleCampaignClaimReview(claim: CampaignClaim, status: "acknowledged" | "rejected") {
    if (isSimulation) { setNotice({ kind: "error", message: "La simulation est en lecture seule. Quittez-la pour traiter une réclamation." }); return; }
    setRequestActionId(claim.id);
    try {
      await reviewCampaignClaim(claim.id, status);
      await refreshUsers(profile);
      setNotice({ kind: "success", message: status === "acknowledged" ? "Réclamation prise en compte." : "Réclamation rejetée." });
    } catch (error) {
      setNotice({ kind: "error", message: readableSupabaseError(error, "Impossible de traiter la réclamation.") });
    } finally {
      setRequestActionId(null);
    }
  }
  ...
  }, [users, search, roleFilter, categoryFilter, campaignFilter, campaignAssignments, campaignSupervisorAssignments]);
  ...
  function campaignNamesForUser(userId: string): string {
    const ids = new Set([...campaignAssignments.filter((assignment) => assignment.user_id === userId && assignment.is_active).map((assignment) => assignment.campaign_id), ...campaignSupervisorAssignments.filter((assignment) => assignment.agent_id === userId && assignment.is_active).map((assignment) => assignment.campaign_id)]);
    return campaigns.filter((campaign) => ids.has(campaign.id)).map((campaign) => campaign.name).join(" · ");
  }
  ...
  function exportCsv() {
    ...
    const rows = filteredUsers.map((user) => [user.id, user.full_name, user.phone, user.role, user.user_category || "", campaignNamesForUser(user.id), supervisorDisplayName(user.supervisor_id, users), user.permanent_shop_id || ""].map(escapeCsv).join(","));
  ...
  async function exportUsersXlsx() {
    ...
    const rows = filteredUsers.map((user) => [user.id, user.full_name, user.phone, ROLE_LABELS[user.role], user.user_category ? categoryShortLabel(user.user_category) : "", campaignNamesForUser(user.id), supervisorDisplayName(user.supervisor_id, users), user.permanent_shop_id || ""]);
...
<div className="signup-category"><span>Catégorie agent</span><CustomSelect value={signup.category} onChange={(value) => setSignup((current) => ({ ...current, category: value as UserCategory }))} ariaLabel="Catégorie agent" placeholder="Catégorie" options={CATEGORY_OPTIONS.map((category) => ({ value: category, label: categoryShortLabel(category) }))} /></div>
    ...
    {configured && isAuthenticated && <>{isReadOnly && <div className="readonly-banner"><ShieldCheck size={15} /><span><strong>Lecture seule</strong> · {ROLE_LABELS[profile!.role]}</span></div>}
      ...
      {canManageCampaigns && campaignClaims.length > 0 && <section className="pending-panel campaign-claims-panel"><div className="pending-heading"><div><div className="card-kicker"><FileText size={14} /> Réclamations campagne</div><h3>Réclamations à traiter</h3><p>Les agents ont signalé un problème de suivi, de présence ou de pointage.</p></div><span className="pending-count">{campaignClaims.length}</span><button type="button" className="icon-button" onClick={() => void refreshUsers()} disabled={loading} aria-label="Actualiser les réclamations" title="Actualiser les réclamations">{loading ? <LoaderCircle className="spin" size={14} /> : <RefreshCw size={14} />}</button></div><div className="pending-list">{campaignClaims.map((claim) => { const agent = users.find((user) => user.id === claim.user_id); const campaign = campaigns.find((item) => item.id === claim.campaign_id); if (!agent || !campaign) return null; return <div className="pending-item claim-pending-item" key={claim.id}><div className="avatar small">{agent.full_name.slice(0, 1).toUpperCase()}</div><div className="pending-identity"><strong>{agent.full_name} · {campaign.name}</strong><small className="claim-pending-description">{claim.description}</small><small>{new Date(claim.created_at).toLocaleDateString("fr-FR")}</small></div><div className="pending-actions"><button type="button" className="icon-action approve" onClick={() => void handleCampaignClaimReview(claim, "acknowledged")} disabled={requestActionId === claim.id} aria-label="Prendre en compte" title="Prendre en compte"><CheckCircle2 size={14} /></button><button type="button" className="icon-action delete" onClick={() => void handleCampaignClaimReview(claim, "rejected")} disabled={requestActionId === claim.id} aria-label="Rejeter" title="Rejeter"><XCircle size={14} /></button></div></div>; })}</div></section>}
