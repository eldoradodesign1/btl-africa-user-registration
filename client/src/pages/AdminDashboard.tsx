  ...
  return <article className={`user-gallery-card ${revealed ? "is-revealed" : ""}`} tabIndex={0} role="button" aria-expanded={revealed} aria-label={`${revealed ? "Ouvrir la fiche de" : "Révéler les informations de"} ${user.full_name}`} onClick={toggleCard} onKeyDown={handleKeyDown}>
    <div className="user-gallery-track">
      ...
      <div className="user-gallery-info" aria-hidden={!revealed}>
        ...
        <div className="user-gallery-facts"><div><small>Campagnes</small><div className="campaign-pills">{userCampaigns.length ? userCampaigns.map((campaign) => <span className="campaign-pill" key={campaign.id} title={campaign.name}>{campaign.name}</span>) : <span className="muted-note">Aucune</span>}</div></div><div><small>Statut</small><strong className={`activity-text ${user.is_active ? "is-active" : "is-inactive"}`}>{user.is_active ? "Actif" : "Inactif"}</strong></div><div><small>Lime</small><strong>{limeDisplayName(user.supervisor_id, users)}</strong></div><div><small>Shop</small><strong>{shopDisplayName(user.permanent_shop_id, shops)}</strong></div></div>
...
function AdminDashboard({ onConnectionChanged, onRequestCreate }: Props) {
  ...
  function exportCsv() {
    if (!selectedExportUsers.length) return;
    const headers = ["id", "full_name", "phone", "role", "user_category", "status", "campaigns", "lime", "permanent_shop_id"];
    const rows = selectedExportUsers.map((user) => [user.id, user.full_name, user.phone, user.role, user.user_category || "", user.is_active ? "Actif" : "Inactif", campaignNamesForUser(user.id), limeDisplayName(user.supervisor_id, users), user.permanent_shop_id || ""].map(escapeCsv).join(","));
  ...
  async function exportUsersXlsx() {
    ...
    const rows = selectedExportUsers.map((user) => [user.id, user.full_name, user.phone, ROLE_LABELS[user.role], user.user_category ? categoryShortLabel(user.user_category) : "", user.is_active ? "Actif" : "Inactif", campaignNamesForUser(user.id), limeDisplayName(user.supervisor_id, users), user.permanent_shop_id || ""]);
  ...
  function exportUsersPdf() {
    ...
    if (!popup) return;
    const rows = selectedExportUsers.map((user) => `<tr><td>${escapeHtml(user.full_name)}</td><td>${escapeHtml(user.phone)}</td><td>${escapeHtml(ROLE_LABELS[user.role])}</td><td>${escapeHtml(user.user_category ? categoryShortLabel(user.user_category) : "—")}</td><td>${escapeHtml(user.is_active ? "Actif" : "Inactif")}</td><td>${escapeHtml(campaignNamesForUser(user.id) || "—")}</td><td>${escapeHtml(user.permanent_shop_id || "—")}</td></tr>`).join("");
    popup.document.write(`<html><head><title>BTL Africa — Utilisateurs</title><style>@page{size:A4 landscape;margin:12mm}body{font-family:Arial,sans-serif;color:#17343a;font-size:11px}header{padding:18px 20px;background:#12383f;color:#fff;border-radius:12px}h1{margin:8px 0 3px;font-size:24px}p{margin:0;color:#c7e2e2}table{width:100%;margin-top:18px;border-collapse:collapse}th{padding:8px;background:#12383f;color:#fff;text-align:left;font-size:9px}td{padding:8px;border-bottom:1px solid #e2eded}tr:nth-child(even){background:#f7fbfa}.footer{margin-top:18px;color:#789095;font-size:9px;text-align:right}</style></head><body><header><div>BTL AFRICA · EXPORT UTILISATEURS</div><h1>${selectedExportUsers.length} utilisateur${selectedExportUsers.length > 1 ? "s" : ""}</h1><p>Campagne : ${escapeHtml(selectedCampaignName)} · Généré le ${new Date().toLocaleString("fr-FR")}</p></header><table><thead><tr><th>Nom complet</th><th>MSISDN</th><th>Rôle</th><th>Catégorie</th><th>Statut</th><th>Campagnes</th><th>Shop</th></tr></thead><tbody>${rows}</tbody></table><div class="footer">BTL Africa · Privilege Tracker</div></body></html>`);
