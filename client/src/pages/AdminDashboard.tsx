...
function UserGalleryCard({ user, campaigns, assignments, shops, canManage, canManageCampaigns, onOpenUser, onOpenPhoto, onAssign, onEdit, onDelete }: UserGalleryCardProps) {
  ...
  return <article className={`user-gallery-card ${revealed ? "is-revealed" : ""}`} tabIndex={0} role="button" aria-expanded={revealed} aria-label={`${revealed ? "Ouvrir la fiche de" : "Révéler les informations de"} ${user.full_name}`} onClick={toggleCard} onKeyDown={handleKeyDown}>
    <div className="user-gallery-track">
      ...
      <div className="user-gallery-info" aria-hidden={!revealed}>
        ...
        <div className="user-gallery-facts"><div><small>Campagnes</small><div className="campaign-pills">{userCampaigns.length ? userCampaigns.map((campaign) => <span className="campaign-pill" key={campaign.id} title={campaign.name}>{campaign.name}</span>) : <span className="muted-note">Aucune</span>}</div></div><div><small>Superviseur</small><strong>{user.supervisor_id ? `${user.supervisor_id.slice(0, 8)}…` : "Aucun"}</strong></div><div><small>Shop</small><strong>{shopDisplayName(user.permanent_shop_id, shops)}</strong></div></div>
...
function AdminDashboard({ onConnectionChanged, onRequestCreate }: Props) {
  ...
  function exportCsv() {
    const headers = ["id", "full_name", "phone", "role", "user_category", "campaigns", "supervisor_id", "permanent_shop_id"];
    const rows = filteredUsers.map((user) => [user.id, user.full_name, user.phone, user.role, user.user_category || "", campaignNamesForUser(user.id), user.supervisor_id || "", user.permanent_shop_id || ""].map(escapeCsv).join(","));
  ...
  async function exportUsersXlsx() {
    ...
    const rows = filteredUsers.map((user) => [user.id, user.full_name, user.phone, ROLE_LABELS[user.role], user.user_category ? categoryShortLabel(user.user_category) : "", campaignNamesForUser(user.id), user.supervisor_id || "", user.permanent_shop_id || ""]);
...
<div className="signup-category"><span>Catégorie agent</span><CustomSelect value={signup.category} onChange={(value) => setSignup((current) => ({ ...current, category: value as UserCategory }))} ariaLabel="Catégorie agent" placeholder="Catégorie" options={CATEGORY_OPTIONS.map((category) => ({ value: category, label: categoryShortLabel(category) }))} /></div>
    ...
    {selectedUserProfile && <UserDetailModal user={selectedUserProfile} superiors={superiors} campaigns={campaigns} assignments={campaignAssignments} assignmentRequests={assignmentRequests} canRequest={false} onNotice={(next) => setNotice(next)} onClose={() => setSelectedUserProfile(null)} />}
