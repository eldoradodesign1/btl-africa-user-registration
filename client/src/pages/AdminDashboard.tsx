  ...
  const refreshUsers = useCallback(async (currentProfile: UserRecord | null = profile, force = false) => {
    ...
    const task = (async () => {
      ...
      try {
        ...
        let nextUserPasswords: Record<string, string | null> = {};
        let nextPasswordAssistanceReady = false;
        if (canViewPasswords) {
          try {
            nextUserPasswords = await loadSuperAdminPasswords();
            nextPasswordAssistanceReady = true;
          } catch {
            nextUserPasswords = {};
          }
        }
        ...
        setUsers(nextUsers);
        setUserPasswords(nextUserPasswords as Record<string, string | null>);
        setPasswordAssistanceReady(nextPasswordAssistanceReady);
