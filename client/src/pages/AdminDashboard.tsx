...
function UserGalleryCard({ user, users, campaignsByUserId, shops, canManage, canManageCampaigns, onOpenUser, onOpenPhoto, onAssign, onEdit, onDelete, canViewActivity, canToggleActivity, activityUpdatingId, onToggleActivity }: UserGalleryCardProps) {
...
function AdminDashboard({ onConnectionChanged, onRequestCreate }: Props) {
  ...
  async function toggleUserActivity(user: UserRecord) {
    ...
    try {
      const updated = await setUserActivityStatus(user.id, !user.is_active);
...
<div className="signup-category"><span>Catégorie agent</span><CustomSelect value={signup.category} onChange={(value) => setSignup((current) => ({ ...current, category: value as UserCategory }))} ariaLabel="Catégorie agent" placeholder="Catégorie" options={CATEGORY_OPTIONS.map((category) => ({ value: category, label: categoryShortLabel(category) }))} /></div>
    ...
    {configured && isAuthenticated && <>{isReadOnly && <div className="readonly-banner"><ShieldCheck size={15} /><span><strong>Lecture seule</strong> · {ROLE_LABELS[profile!.role]}</span></div>}
      ...
      {canManageCampaigns && campaignClaims.length > 0 && <section className="pending-panel campaign-claims-panel"><div className="pending-heading queue-heading" role="button" tabIndex={0} aria-expanded={expandedQueues.claims} onClick={() => toggleQueue("claims")} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); toggleQueue("claims"); } }}><div className="queue-heading-main"><div><div className="card-kicker"><FileText size={14} /> Réclamations campagne</div><h3>Dossiers à traiter</h3><p>Ouvrez un dossier pour répondre, demander des informations ou le clôturer.</p></div></div><span className="pending-count">{campaignClaims.length}</span><ChevronDown className={`queue-chevron ${expandedQueues.claims ? "is-open" : ""}`} size={15} /><button type="button" className="icon-button" onClick={(event) => { event.stopPropagation(); void refreshUsers(profile, true); }} disabled={loading} aria-label="Actualiser les dossiers" title="Actualiser les dossiers">{loading ? <LoaderCircle className="spin" size={14} /> : <RefreshCw size={14} />}</button></div>{expandedQueues.claims && <div className="pending-list">{campaignClaims.map((claim) => { const agent = users.find((user) => user.id === claim.user_id); const campaign = campaigns.find((item) => item.id === claim.campaign_id); if (!agent || !campaign) return null; return <button type="button" className="pending-item claim-pending-item" key={claim.id} onClick={() => setSelectedClaim(claim)}><div className="avatar small">{agent.full_name.slice(0, 1).toUpperCase()}</div><div className="pending-identity"><strong>{agent.full_name} · {campaign.name}</strong><small>{CLAIM_STATUS_NAMES[claim.status]} · {new Date(claim.updated_at || claim.created_at).toLocaleDateString("fr-FR")}</small><span className="claim-pending-description">{claim.description}</span></div><span className="button secondary compact">Ouvrir le dossier</span></button>; })}</div>}</section>}
