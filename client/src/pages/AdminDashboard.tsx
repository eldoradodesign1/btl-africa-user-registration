...
import { AlertCircle, BriefcaseBusiness, CheckCircle2, ChevronDown, Clock3, Database, Download, FilePenLine, FileSpreadsheet, FileText, Filter, LayoutGrid, List, LoaderCircle, LogIn, LogOut, PieChart, RefreshCw, Search, ServerCog, ShieldCheck, Trash2, UserCheck, UserPlus, UserCircle2, UserRoundPlus, X, XCircle } from "lucide-react";
import { CopyablePhone } from "@/components/CopyablePhone";
import RoleWorkspace, { AgentDetailModal, Avatar, ProfileModal } from "@/components/RoleWorkspace";
...
function AdminDashboard({ onConnectionChanged, onRequestCreate }: Props) {
  ...
  async function exportUsersXlsx() {
    ...
    const rows = filteredUsers.map((user) => [user.id, user.full_name, user.phone, ROLE_LABELS[user.role], user.user_category ? categoryShortLabel(user.user_category) : "", user.supervisor_id || "", user.permanent_shop_id || ""]);
    const sheet = XLSX.utils.aoa_to_sheet([["BTL AFRICA · UTILISATEURS", "", "", "", "", "", ""], ["ID", "Nom complet", "MSISDN", "Rôle", "Catégorie", "Superviseur", "Shop"], ...rows]);
    sheet["!merges"] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 6 } }];
    sheet["!cols"] = [22, 28, 17, 18, 28, 24, 18].map((wch) => ({ wch }));
    sheet["!freeze"] = { xSplit: 0, ySplit: 2 };
    sheet["A1"].s = { fill: { fgColor: { rgb: "12383F" } }, font: { color: { rgb: "FFFFFF" }, bold: true, sz: 13 } };
    ["A2", "B2", "C2", "D2", "E2", "F2", "G2"].forEach((cell) => { sheet[cell].s = { fill: { fgColor: { rgb: "9EE9E8" } }, font: { color: { rgb: "082126" }, bold: true } }; });
    XLSX.utils.book_append_sheet(workbook, sheet, "Utilisateurs");
    const summary = XLSX.utils.aoa_to_sheet([["INDICATEURS", "VALEUR"], ["Utilisateurs exportés", { f: `COUNTA(Utilisateurs!B3:B${rows.length + 2})` }], ["Agents", { f: `COUNTIF(Utilisateurs!D3:D${rows.length + 2},\"Agent\")` }], ["Administratifs", { f: `B2-B3` }]]);
  ...
  function exportUsersPdf() {
    ...
    if (!popup) return;
    const rows = filteredUsers.map((user) => `<tr><td>${escapeHtml(user.full_name)}</td><td>${escapeHtml(user.phone)}</td><td>${escapeHtml(ROLE_LABELS[user.role])}</td><td>${escapeHtml(user.user_category ? categoryShortLabel(user.user_category) : "—")}</td><td>${escapeHtml(user.permanent_shop_id || "—")}</td></tr>`).join("");
    popup.document.write(`<html><head><title>BTL Africa — Utilisateurs</title><style>@page{size:A4 landscape;margin:12mm}body{font-family:Arial,sans-serif;color:#17343a;font-size:11px}header{padding:18px 20px;background:#12383f;color:#fff;border-radius:12px}h1{margin:8px 0 3px;font-size:24px}p{margin:0;color:#c7e2e2}table{width:100%;margin-top:18px;border-collapse:collapse}th{padding:8px;background:#12383f;color:#fff;text-align:left;font-size:9px}td{padding:8px;border-bottom:1px solid #e2eded}tr:nth-child(even){background:#f7fbfa}.footer{margin-top:18px;color:#789095;font-size:9px;text-align:right}</style></head><body><header><div>BTL AFRICA · EXPORT UTILISATEURS</div><h1>${filteredUsers.length} utilisateur${filteredUsers.length > 1 ? "s" : ""}</h1><p>Généré le ${new Date().toLocaleString("fr-FR")}</p></header><table><thead><tr><th>Nom complet</th><th>MSISDN</th><th>Rôle</th><th>Catégorie</th><th>Shop</th></tr></thead><tbody>${rows}</tbody></table><div class="footer">BTL Africa · Privilege Tracker</div></body></html>`);
  ...
  return <section className="admin-dashboard glass-card">{simulationControl}
    <div className="dashboard-header"><div className="dashboard-title"><div className="heading-icon"><ServerCog size={19} /></div><div><div className="eyebrow"><ShieldCheck size={13} /> Console sécurisée</div><h2>Tableau de bord administrateur</h2></div></div><div className="dashboard-actions">{effectiveProfile && <span className="session-chip"><span className="session-dot" />{effectiveProfile.full_name} · {effectiveProfile.role === "super_admin" ? "Super-administrateur" : ROLE_LABELS[effectiveProfile.role]}</span>}{profile && <button className="icon-button" type="button" onClick={() => void handleLogout()} aria-label="Se déconnecter" title="Se déconnecter"><LogOut size={15} /></button>}{configured && profile?.role === "super_admin" && !isSimulation && <button className="icon-button" type="button" onClick={() => setShowConfig(true)} aria-label="Configurer la base de données" title="Configurer la base de données"><Database size={16} /></button>}</div></div>{effectiveProfile && <div className="admin-profile-hero"><div className="admin-profile-photo"><Avatar user={effectiveProfile || profile!} size="large" /><button type="button" className="workspace-profile-edit" onClick={() => isSimulation ? setNotice({ kind: "error", message: "Le profil est indisponible pendant une simulation." }) : setProfileOpen(true)} aria-label="Modifier mon profil" title="Modifier mon profil"><FilePenLine size={13} /></button></div><div><div className="eyebrow"><UserCircle2 size={13} /> Profil actif</div><h3>{effectiveProfile?.full_name || "Utilisateur"}</h3><p>{effectiveProfile ? ROLE_LABELS[effectiveProfile.role] : ""} · Gérez vos informations personnelles depuis la photo.</p></div></div>}
