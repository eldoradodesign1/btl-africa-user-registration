...
function UserGalleryCard({ user, users, campaigns, assignments, shops, canManage, canManageCampaigns, onOpenUser, onOpenPhoto, onAssign, onEdit, onDelete }: UserGalleryCardProps) {
  const [revealed, setRevealed] = useState(false);
  const userCampaigns = assignments
    .filter((assignment) => assignment.user_id === user.id && assignment.is_active)
    .map((assignment) => campaigns.find((campaign) => campaign.id === assignment.campaign_id))
  ...
  return <article className={`user-gallery-card ${revealed ? "is-revealed" : ""}`} tabIndex={0} role="button" aria-expanded={revealed} aria-label={`${revealed ? "Ouvrir la fiche de" : "Révéler les informations de"} ${user.full_name}`} onClick={toggleCard} onKeyDown={handleKeyDown}>
    <div className="user-gallery-track">
      ...
      <div className="user-gallery-info" aria-hidden={!revealed}>
        ...
        <div className="user-gallery-facts"><div><small>Campagnes</small><div className="campaign-pills">{userCampaigns.length ? userCampaigns.map((campaign) => <span className="campaign-pill" key={campaign.id} title={campaign.name}>{campaign.name}</span>) : <span className="muted-note">Aucune</span>}</div></div><div><small>Superviseur</small><strong>{supervisorDisplayName(user.supervisor_id, users)}</strong></div><div><small>Shop</small><strong>{shopDisplayName(user.permanent_shop_id, shops)}</strong></div></div>
...
function AdminDashboard({ onConnectionChanged, onRequestCreate }: Props) {
  ...
  async function refreshUsers(currentProfile: UserRecord | null = profile) {
    setLoading(true);
    try {
      const [nextUsers, nextSuperiors, nextCampaigns, nextShops, nextAssignments, nextRequests, nextClaims] = await Promise.all([loadUsers(), loadSupervisors(), loadCampaigns(), loadShops().catch(() => []), loadCampaignAssignments(), currentProfile && ["admin", "super_admin", "sub_admin", "supervisor"].includes(currentProfile.role) ? loadCampaignAssignmentRequests() : Promise.resolve([]), currentProfile && ["admin", "super_admin", "sub_admin", "supervisor"].includes(currentProfile.role) ? loadCampaignClaims().catch(() => []) : Promise.resolve([])]);
      setUsers(nextUsers);
  ...
  const filteredUsers = useMemo(() => {
    ...
    const campaignUserIds = campaignFilter === "all" ? null : new Set(campaignAssignments.filter((assignment) => assignment.campaign_id === campaignFilter && assignment.is_active).map((assignment) => assignment.user_id));
  ...
  }, [users, search, roleFilter, categoryFilter, campaignFilter, campaignAssignments]);
  ...
  function campaignNamesForUser(userId: string): string {
    return campaignAssignments.filter((assignment) => assignment.user_id === userId && assignment.is_active).map((assignment) => campaigns.find((campaign) => campaign.id === assignment.campaign_id)?.name).filter((name): name is string => Boolean(name)).join(" · ");
  }
...
<div className="signup-category"><span>Catégorie agent</span><CustomSelect value={signup.category} onChange={(value) => setSignup((current) => ({ ...current, category: value as UserCategory }))} ariaLabel="Catégorie agent" placeholder="Catégorie" options={CATEGORY_OPTIONS.map((category) => ({ value: category, label: categoryShortLabel(category) }))} /></div>
    ...
    {selectedAgentProfile && <AgentDetailModal agent={selectedAgentProfile} users={users} campaigns={campaigns} assignments={campaignAssignments} assignmentRequests={assignmentRequests} canRequest={false} canExport canEdit={canManage} onEdit={() => { if (selectedAgentProfile) beginEdit(selectedAgentProfile); setSelectedAgentProfile(null); }} onNotice={(next) => setNotice(next)} onClose={() => setSelectedAgentProfile(null)} />}
    {selectedUserProfile && <UserDetailModal user={selectedUserProfile} users={users} superiors={superiors} campaigns={campaigns} assignments={campaignAssignments} assignmentRequests={assignmentRequests} canRequest={false} onNotice={(next) => setNotice(next)} onClose={() => setSelectedUserProfile(null)} />}
