...
L’assistance de mot de passe est disponible uniquement pour le **superadmin réel**, jamais en simulation. Le bouton **Afficher les mots de passe** demande le mot de passe actuel du superadmin à chaque déverrouillage, puis affiche les valeurs existantes dans la liste, la galerie et la fiche agent. Les requêtes utilisateurs ordinaires continuent d’exclure `password_hash`; les valeurs d’assistance restent uniquement en mémoire et sont effacées à la déconnexion, à l’entrée en simulation ou au masquage. La migration [`supabase/migrations/202609240006_lock_password_assistance.sql`](supabase/migrations/202609240006_lock_password_assistance.sql) verrouille également la RPC contre les identifiants ou mots de passe invalides.
