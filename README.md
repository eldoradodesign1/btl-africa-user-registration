...
La migration [`supabase/migrations/202609210005_secure_password_storage.sql`](supabase/migrations/202609210005_secure_password_storage.sql) doit être exécutée après les migrations précédentes. Elle active `pgcrypto`, convertit les anciennes valeurs `password_hash` en hachages bcrypt, fait passer la connexion par `authenticate_user`, hache les nouveaux mots de passe côté PostgreSQL et exige le mot de passe actuel avant toute modification de profil. Le frontend ne sélectionne ni ne renvoie jamais `password_hash`.
