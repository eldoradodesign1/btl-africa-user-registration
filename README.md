...
La migration [`supabase/migrations/202609230001_campaign_supervisor_assignments.sql`](supabase/migrations/202609230001_campaign_supervisor_assignments.sql) ajoute la relation opérationnelle `agent_campaign_supervisor_assignments`. Elle conserve `users.supervisor_id` comme rattachement historique d’entrée dans l’agence, mais permet désormais plusieurs superviseurs pour un même agent et une même campagne. Elle synchronise encore `user_campaign_assignments` pour préserver les rapports existants, filtre les demandes d’affectation vers le superviseur opérationnel concerné et prépare le compte administratif temporaire `Sam` demandé pour le reclassement initial. Les superviseurs ne voient et ne modifient que leurs agents opérationnels ; la coordination, les admins et les superadmins disposent d’une vue globale.

Dans l’espace agent, les campagnes sont maintenant choisies dans un sélecteur unique placé au-dessus du suivi. La courbe et le calendrier ne sont chargés que pour une campagne affectée ; une campagne non affectée affiche uniquement la possibilité de demander l’affectation. Pour activer les réclamations, exécutez ensuite [`supabase/migrations/202609220012_campaign_claims.sql`](supabase/migrations/202609220012_campaign_claims.sql). Cette migration crée `campaign_claims` et les RPC permettant à un agent affecté d’envoyer une réclamation, puis à son superviseur ou à un rôle administratif de la prendre en compte ou de la rejeter.
