...
Pour l’assistance opérationnelle, le compte **super_admin réel uniquement** dispose d’un accès visuel aux valeurs de mot de passe existantes dans la liste et dans la fiche agent, après authentification réelle. Les chargements utilisateurs ordinaires continuent d’exclure `password_hash`; la lecture d’assistance passe par [`supabase/migrations/202609240001_superadmin_password_assistance.sql`](supabase/migrations/202609240001_superadmin_password_assistance.sql), puis [`supabase/migrations/202609240002_superadmin_password_assistance_reauth.sql`](supabase/migrations/202609240002_superadmin_password_assistance_reauth.sql), qui exige le mot de passe courant du superadmin et refuse les autres rôles. Le mot de passe du superadmin reste uniquement en mémoire pendant sa session et le mode simulation efface immédiatement les valeurs d’assistance de l’interface.

Le dashboard se synchronise avec les changements Realtime Supabase sur les tables métier principales. La publication `supabase_realtime` est gérée par la migration [`202609230008_enable_realtime_tables.sql`](supabase/migrations/202609230008_enable_realtime_tables.sql), de manière idempotente. Une resynchronisation complète est déclenchée uniquement au retour sur l’onglet, afin de couvrir la suspension éventuelle d’une connexion WebSocket sans installer de polling permanent.
